...
for i=1:prod(phenotypeFactor.card),
	getassign = IndexToAssignment(i,phenotypeFactor.card);
	probb = alphaList(allelesToGenotypes(getassign(2),getassign(3)));
	if (getassign(1)==1),
		phenotypeFactor.val(i) = probb;
	else 
		phenotypeFactor.val(i) = 1-probb;
	end
end
